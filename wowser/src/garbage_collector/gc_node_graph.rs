...
impl<T: GarbageCollectable> GcNodeGraph<T> {
    ...
    pub fn new(root_value: T) -> (Self, GcNode<T>) {
        let strong_node = Rc::new(RefCell::new(InnerNode::new(root_value)));
        let root_node = GcNode::new(Rc::downgrade(&strong_node));
        (
            GcNodeGraph {
                nodes: vec![],
                root: strong_node,
    ...
    pub fn create_node(&mut self, value: T) -> GcNode<T> {
    ...
    fn mark_nodes(root_node: &Rc<RefCell<InnerNode<T>>>, new_gc_sweep_id: GcSweepId) {
        let mut queue: Vec<GcNode<T>> = vec![GcNode::new(Rc::downgrade(root_node))];
